...
async function checkPendingStatus() {
  if (!agentConfig.token || agentConfig.pendingUploads.size === 0) return;
...
async function uploadFile(filePath) {
  ...
  try {
    ...
    log('Upload failed: ' + (response.data?.message || 'Unknown'), 'ERROR');
    return null;
  } catch (err) {
    log('Upload error: ' + (err.response?.data?.message || err.message), 'ERROR');
    return null;
  }
}
